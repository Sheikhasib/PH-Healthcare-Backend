...
const verifyDoctorEmail = async (payload: IVerifyDoctorEmailPayload) => {
  ...
  if (!redisOTP) {
    throw new AppError(httpStatus.BAD_REQUEST, "OTP expired. Your application has been rejected.");
  }

  if (redisOTP !== otp) {
    throw new AppError(httpStatus.BAD_REQUEST, "OTP does not match. Please try again.");
  }
...
const approveDoctor = async (
  ...
  if (existingDoctor.isDeleted) {
    throw new AppError(httpStatus.NOT_FOUND, "Doctor application already deleted");
  }
